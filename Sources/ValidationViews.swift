    ...
    VStack(alignment: .leading, spacing: 10) {
      ...
      if let store, let linkedOrder {
        ...
        if !mailboxSummaries.isEmpty {
          ValidationMailboxSourceTrail(summaries: mailboxSummaries)
        }
      }
...
private struct ValidationMailboxSourceTrail: View {
  ...
  var body: some View {
    ...
    VStack(alignment: .leading, spacing: 7) {
      ...
      if summaries.count > displayedSummaries.count {
        Text("\(summaries.count - displayedSummaries.count) more mailbox source summaries hidden for this validation issue.")
          .font(.caption2)
          .foregroundStyle(.secondary)
      }
    }
