...
struct ProcurementView: View {
  ...
  var body: some View {
    ScrollView {
      VStack(alignment: .leading, spacing: 16) {
        ...
        gmailProcurementReadinessPanel
  ...
  private var inboxProcurementCoverage: some View {
    SettingsPanel(title: "Inbox and Wishlist procurement readiness", symbol: "cart.badge.plus") {
      ...
      if !procurementProviderRows.isEmpty {
        VStack(alignment: .leading, spacing: 8) {
          Text("Mailbox source for procurement")
            .font(.caption.weight(.semibold))
            .foregroundStyle(.secondary)
          LazyVGrid(columns: [GridItem(.adaptive(minimum: 250), spacing: 10)], spacing: 10) {
            ForEach(procurementProviderRows, id: \.label) { row in
  ...
  private var procurementProviderRows: [(label: String, count: Int, detail: String, symbol: String, color: Color)] {
    ...
    return counts.map { label, count in
      ...
      switch tone {
      case "spacemail":
        detail = "SpaceMail intake can create replacement, supplier, or buying follow-up after an Inbox order is linked or created."
      case "gmail":
        detail = "Gmail intake can create replacement, supplier, or buying follow-up after an Inbox order is linked or created."
      case "mock":
        detail = "Mock mailbox intake supports local procurement testing. Confirm live provider context before handoff."
      default:
        detail = "Local mailbox intake can create procurement follow-up once linked to an order."
      }
      return (
        label: label,
        count: count,
  ...
  private var gmailProcurementSourceCount: Int {
    procurementProviderRows
      .filter { $0.label.localizedCaseInsensitiveContains("Gmail") }
      .reduce(0) { total, row in total + row.count }
  }

  private var microsoft365ProcurementSourceCount: Int {
    procurementProviderRows
    ...
    VStack(alignment: .leading, spacing: 10) {
      ...
      if let store, let linkedOrder {
        OrderMailboxSourceTrailPanel(
          ...
          title: "Mailbox provider procurement trail",
  ...
  private var procurementInboxSourceTrail: some View {
    VStack(alignment: .leading, spacing: 6) {
      ...
      if linkedWishlistItems.count > 2 || linkedIntakeEmails.count > 3 {
        Text("\(max(linkedWishlistItems.count - 2, 0)) more Wishlist item\(max(linkedWishlistItems.count - 2, 0) == 1 ? "" : "s") and \(max(linkedIntakeEmails.count - 3, 0)) more source email\(max(linkedIntakeEmails.count - 3, 0) == 1 ? "" : "s") are linked to this procurement request but hidden in the compact grid.")
          .font(.caption2)
          .foregroundStyle(.secondary)
      }
    }
