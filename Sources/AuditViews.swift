...
struct AuditView: View {
  ...
  private var auditNextCheckTitle: String {
    ...
    if !inboxDispatchHandoffEvents.isEmpty {
      return "Confirm source dispatch handoff trail"
    }
  ...
  private var auditNextCheckDetail: String {
    ...
    if !inboxDispatchHandoffEvents.isEmpty {
      return "Check reopened and completed dispatch handoff events together so Inbox-created or Wishlist-linked order follow-up does not get lost across Orders, Dispatch, and Tasks."
    }
    if !wishlistHandoffSanityItems.isEmpty {
  ...
  private var auditEvidenceItems: [(title: String, detail: String, count: Int, symbol: String, color: Color)] {
    [
      ...
      (
        ...
        "Dispatch actions show whether Inbox-created or Wishlist-linked orders gained manifest/readiness context.",
      ...
      VStack(alignment: .leading, spacing: 12) {
        ...
        if !inboxCreatedOrdersWithMailboxSourceTrail.isEmpty {
          VStack(alignment: .leading, spacing: 8) {
            ...
            Text("These Inbox-created or Wishlist-linked orders can be traced back to the mailbox provider that supplied their intake rows.")
              .font(.caption)
              .foregroundStyle(.secondary)
              .fixedSize(horizontal: false, vertical: true)
  ...
  private var inboxDispatchHandoffTrailPanel: some View {
    if !inboxDispatchHandoffEvents.isEmpty {
      SettingsPanel(title: "Source dispatch handoff trail", symbol: "arrow.triangle.2.circlepath.circle.fill") {
        VStack(alignment: .leading, spacing: 12) {
          Text("Recent local events that connect Inbox-created or Wishlist-linked orders to Dispatch and Tasks. Use this trail when a handoff is reopened, completed, or resolved locally.")
            .font(.subheadline)
            .foregroundStyle(.secondary)
            .fixedSize(horizontal: false, vertical: true)
    ...
    SettingsPanel(title: "Activity feed", symbol: "list.clipboard.fill") {
      VStack(alignment: .leading, spacing: 14) {
        ...
        if recentEvents.isEmpty {
          ...
          AuditFeedSection(title: "Source dispatch handoff trail", detail: "Reopened, completed, blocked, and resolved dispatch follow-up for Inbox-created or Wishlist-linked orders.", events: visibleInboxDispatchHandoffEvents, onCreateTask: { event in
            store.createReviewTask(from: event)
          })
...
private extension AuditEvent {
  ...
  var inboxDispatchHandoffTrailGuidance: String {
    ...
    if summary.localizedCaseInsensitiveContains("blocked") {
      return "Resolve the blocked manifest or readiness checklist before treating the Inbox-created or Wishlist-linked order as dispatch-ready."
    }
    ...
    return "Use this event to trace how an Inbox-created or Wishlist-linked order moved through local dispatch follow-up."
  }
