      ...
      if !inboxOrders.isEmpty {
        VStack(alignment: .leading, spacing: 6) {
          ...
          if inboxOrders.count > 2 {
            Text("\(inboxOrders.count - 2) more Inbox/Wishlist order\(inboxOrders.count - 2 == 1 ? "" : "s") are linked to this vendor profile but hidden in the compact row.")
              .font(.caption2)
              .foregroundStyle(.secondary)
          }
          if let store {
            ...
            if emails.count > 2 {
              Text("\(emails.count - 2) more source email\(emails.count - 2 == 1 ? "" : "s") are linked to this vendor profile but hidden here.")
                .font(.caption2)
                .foregroundStyle(.secondary)
            }
          }
