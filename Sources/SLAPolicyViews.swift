...
struct SLAPoliciesView: View {
  ...
  private func slaPolicySearchParts(_ policy: SLAPolicy) -> [String] {
    let mailboxSummaries = inboxOrders(for: policy).flatMap { store.mailboxSourceSummaries(for: $0) }
    var parts = [
    ...
    VStack(alignment: .leading, spacing: 10) {
        ...
        VStack(alignment: .leading, spacing: 6) {
          ...
          if !inboxOrders.isEmpty {
            VStack(alignment: .leading, spacing: 6) {
              ...
              if inboxOrders.count > 2 {
                Text("\(inboxOrders.count - 2) more Inbox/Wishlist order\(inboxOrders.count - 2 == 1 ? "" : "s") are linked to this SLA policy but hidden in the compact row.")
                  .font(.caption2)
                  .foregroundStyle(.secondary)
              }
              if let store {
                ...
                if emails.count > 2 {
                  Text("\(emails.count - 2) more source email\(emails.count - 2 == 1 ? "" : "s") are linked to this SLA policy but hidden here.")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                }
              }
          ...
          if let store {
            OrderMailboxSourceTrailPanel(
              ...
              title: "Mailbox provider SLA trail",
              symbol: "timer"
            )
          }
      ...
      CompactActionRow {
        ...
        Button("Draft", systemImage: "envelope.open.fill") {
          onCreateDraft()
          feedbackMessage = "Draft message created from this SLA policy. It remains local until a person sends anything outside ParcelNest."
        }
          .buttonStyle(.bordered)
