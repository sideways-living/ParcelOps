...
struct ImportQueueView: View {
  ...
  private var gmailReleaseImportConnection: GmailMailboxConnection? {
  ...
  private var gmailReleaseImportColor: Color {
    ...
    return SettingsPanel(title: "Inbox import readiness", symbol: "link.badge.plus") {
      VStack(alignment: .leading, spacing: 12) {
        ...
        if !gmailReleaseSelfChecks.isEmpty {
          VStack(alignment: .leading, spacing: 8) {
            Label("Gmail release boundary before staging", systemImage: gmailReleaseBlockingCount > 0 ? "exclamationmark.shield.fill" : "checkmark.seal.fill")
              .font(.subheadline.weight(.semibold))
              .foregroundStyle(gmailReleaseImportColor)
            Text("Gmail release checks are provider-readiness work. Import Queue should only receive Gmail work after a real or mock Gmail row is imported into Inbox and intentionally staged.")
              .font(.caption)
              .foregroundStyle(.secondary)
              .fixedSize(horizontal: false, vertical: true)
            ...
            if gmailReleaseBlockingCount > 0 || gmailReleaseAttentionCount > 0 {
              CompactActionRow {
                ...
                if let connection = gmailReleaseImportConnection {
          ...
          .padding(10)
          .background(gmailReleaseImportColor.opacity(0.08), in: RoundedRectangle(cornerRadius: 8))
        }
