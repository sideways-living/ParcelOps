    ...
    VStack(alignment: .leading, spacing: 10) {
      ...
      CompactActionRow {
        ...
        Button("Draft", systemImage: "envelope.open.fill") {
          onCreateDraft()
          feedbackMessage = "Draft message created from this inspection. It remains local until a person sends anything outside ParcelNest."
        }
          .buttonStyle(.bordered)
        if let store, let linkedOrder {
          NavigationLink {
  ...
  private var receivingInspectionInboxSourceTrail: some View {
    VStack(alignment: .leading, spacing: 6) {
      ...
      if linkedWishlistItems.count > 2 || linkedIntakeEmails.count > 3 {
        Text("\(max(linkedWishlistItems.count - 2, 0)) more Wishlist item\(max(linkedWishlistItems.count - 2, 0) == 1 ? "" : "s") and \(max(linkedIntakeEmails.count - 3, 0)) more source email\(max(linkedIntakeEmails.count - 3, 0) == 1 ? "" : "s") are linked to this inspection but hidden in the compact grid.")
          .font(.caption2)
          .foregroundStyle(.secondary)
      }
    }
