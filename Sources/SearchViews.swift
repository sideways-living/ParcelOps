...
private struct SearchReadinessPanel: View {
  ...
  private var gmailReleaseSearchConnection: GmailMailboxConnection? {
  ...
  private var gmailReleaseSearchColor: Color {
  ...
  private var providerReleaseNeedsReview: Bool {
    store.mailboxProviderReleaseGateSummary.tone != "success"
      ...
      || gmailReleaseBlockingCount > 0
      || gmailReleaseAttentionCount > 0
  }
        ...
        if providerReleaseNeedsReview {
          VStack(alignment: .leading, spacing: 8) {
            ...
            if !gmailReleaseSelfChecks.isEmpty {
              VStack(alignment: .leading, spacing: 10) {
                Label("Gmail search recovery checks", systemImage: gmailReleaseBlockingCount > 0 ? "exclamationmark.shield.fill" : "checkmark.seal.fill")
                  .font(.subheadline.weight(.semibold))
                  .foregroundStyle(gmailReleaseSearchColor)
                Text("Use this when a Gmail order email is missing from Search results. Setup, sign-in, labels, classifier review, Inbox handoff, and Audit evidence must be visible before a Gmail test pass is considered complete.")
                  .font(.caption)
                  .foregroundStyle(.secondary)
                  .fixedSize(horizontal: false, vertical: true)
                MetricStrip(items: [
                ...
                if gmailReleaseBlockingCount > 0 || gmailReleaseAttentionCount > 0 {
                  CompactActionRow {
                    ...
                    if let connection = gmailReleaseSearchConnection {
              ...
              .padding(10)
              .background(gmailReleaseSearchColor.opacity(0.08), in: RoundedRectangle(cornerRadius: 8))
            }
          }
        }
