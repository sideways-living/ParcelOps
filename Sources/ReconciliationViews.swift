...
struct ReconciliationView: View {
  ...
  private var gmailReleaseReconciliationConnection: GmailMailboxConnection? {
  ...
  private var gmailReleaseReconciliationColor: Color {
        ...
        VStack(alignment: .leading, spacing: 12) {
          ...
          if !gmailReleaseSelfChecks.isEmpty {
            VStack(alignment: .leading, spacing: 10) {
              Label("Gmail reconciliation readiness", systemImage: gmailReleaseBlockingCount > 0 ? "exclamationmark.shield.fill" : "checkmark.seal.fill")
                .font(.subheadline.weight(.semibold))
                .foregroundStyle(gmailReleaseReconciliationColor)
              Text("Gmail self-checks should be complete before Gmail-derived source values, order links, tracking values, or acceptance handoffs are treated as reconciled release evidence.")
                .font(.caption)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
              MetricStrip(items: [
              ...
              if gmailReleaseBlockingCount > 0 || gmailReleaseAttentionCount > 0 {
                CompactActionRow {
                  ...
                  if let connection = gmailReleaseReconciliationConnection {
            ...
            .padding(10)
            .background(gmailReleaseReconciliationColor.opacity(0.08), in: RoundedRectangle(cornerRadius: 8))
          }
        }
      }
    }
