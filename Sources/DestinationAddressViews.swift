...
struct DestinationAddressesView: View {
  ...
  private func destinationAddressSearchParts(_ address: DestinationAddressRecord) -> [String] {
    ...
    let mailboxSummaries = inboxOrders(for: address).flatMap { store.mailboxSourceSummaries(for: $0) }
    ...
    VStack(alignment: .leading, spacing: 10) {
      ...
      if !inboxOrders.isEmpty {
        VStack(alignment: .leading, spacing: 6) {
          ...
          if inboxOrders.count > 2 {
            Text("\(inboxOrders.count - 2) more Inbox/Wishlist order\(inboxOrders.count - 2 == 1 ? "" : "s") are linked to this destination but hidden in the compact row.")
              .font(.caption2)
              .foregroundStyle(.secondary)
          }
          if let store {
            ...
            if emails.count > 2 {
              Text("\(emails.count - 2) more source email\(emails.count - 2 == 1 ? "" : "s") are linked to this destination but hidden here.")
                .font(.caption2)
                .foregroundStyle(.secondary)
            }
          }
      ...
      if let store {
        OrderMailboxSourceTrailPanel(
          ...
          title: "Mailbox provider destination trail",
