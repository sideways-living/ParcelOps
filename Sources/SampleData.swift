...
enum SampleData {
  ...
  static var wishlistPurchaseAccountRecords: [WishlistPurchaseAccountRecord] = [
    ...
    WishlistPurchaseAccountRecord(
      ...
      purchaseBoundaryNote: "Use this only as a local readiness note before buying outside ParcelNest.",
      createdDate: "Sample only",
      lastReviewedDate: "Not reviewed",
      reviewState: .monitor,
  ...
  static var wishlistPurchaseLinkRecords: [WishlistPurchaseLinkRecord] = [
    WishlistPurchaseLinkRecord(
      ...
      accountContext: "Operations purchasing account to confirm outside ParcelNest",
      selectedForPurchase: true,
      createdDate: "Sample only",
      lastCheckedDate: "Sample only",
      reviewState: .needsReview,
      notes: "Direct product link sample only. ParcelNest does not open checkout, log in, buy, pay, or monitor retailer pages."
    ),
    WishlistPurchaseLinkRecord(
      wishlistItemID: nil,
      itemName: "Thermal label rolls",
  ...
  static var communicationTemplates: [CommunicationTemplate] = [
    ...
    CommunicationTemplate(
      ...
      bodyTemplate: "Hi team,\n\nParcelNest has flagged {{record}} for local review. Please confirm the next action and update the tracked record.\n\nThanks.",
  ...
  static var draftMessages: [DraftMessage] = [
    ...
    DraftMessage(
      ...
      body: "Hi team,\n\nParcelNest has flagged SHP-8831 for local review. Please confirm the next action and update the tracked record.\n\nThanks.",
      channel: .internalNote,
  ...
  static var scanSessionRecords: [ScanSessionRecord] = [
    ScanSessionRecord(
      title: "Exception carton manual label check",
      linkedEntityType: .labelReference,
      linkedEntityID: labelReferenceRecords[0].id.uuidString,
      scanPurpose: .labelVerification,
      scanMethodPlaceholder: .manualEntry,
      expectedLabelReferenceValue: labelReferenceRecords[0].labelValuePlaceholder,
      capturedValuePlaceholder: "Damaged label partially unreadable",
      linkedLabelReferenceID: labelReferenceRecords[0].id,
      scanStatus: .mismatchNeedsReview,
      mismatchSummary: "Captured value does not fully match expected tracking label.",
      assignedOperatorTeam: "Receiving Desk",
      scanLocationStorageLocationID: storageLocations[0].id,
      custodyRecordID: custodyRecords[0].id,
      inventoryReceiptID: inventoryReceipts[0].id,
      orderID: orders[0].id,
      shipmentGroupID: shipmentGroups.first?.id,
      packageContentID: packageContents[0].id,
      evidenceAttachmentIDs: evidenceAttachments.prefix(1).map(\.id),
      createdDate: "Today 10:45 AM",
      completedDate: "Not completed",
      notes: "Local scan verification placeholder. No scanner hardware or camera was used.",
      riskLevel: .high,
      reviewState: .needsReview
    ),
    ScanSessionRecord(
      title: "Packing bench shelf QR review",
      linkedEntityType: .labelReference,
      linkedEntityID: labelReferenceRecords[1].id.uuidString,
      scanPurpose: .inventoryHandoff,
      scanMethodPlaceholder: .labelReview,
      expectedLabelReferenceValue: labelReferenceRecords[1].labelValuePlaceholder,
      capturedValuePlaceholder: labelReferenceRecords[1].labelValuePlaceholder,
      linkedLabelReferenceID: labelReferenceRecords[1].id,
      scanStatus: .completed,
      mismatchSummary: "No mismatch recorded.",
      assignedOperatorTeam: "ParcelOps Operations",
      scanLocationStorageLocationID: storageLocations[1].id,
      custodyRecordID: custodyRecords[1].id,
      inventoryReceiptID: inventoryReceipts[1].id,
      orderID: orders.indices.contains(1) ? orders[1].id : nil,
      shipmentGroupID: shipmentGroups.first?.id,
      packageContentID: packageContents[1].id,
      evidenceAttachmentIDs: [],
      createdDate: "Yesterday 3:55 PM",
      completedDate: "Yesterday 4:00 PM",
      notes: "Verified locally from existing label reference.",
      riskLevel: .low,
      reviewState: .accepted
    ),
    ScanSessionRecord(
      title: "Replacement custody scan missing",
      linkedEntityType: .custodyRecord,
      linkedEntityID: custodyRecords[2].id.uuidString,
      scanPurpose: .custodyTransfer,
      scanMethodPlaceholder: .operatorAttestation,
      expectedLabelReferenceValue: labelReferenceRecords[2].labelValuePlaceholder,
      capturedValuePlaceholder: "",
      linkedLabelReferenceID: labelReferenceRecords[2].id,
      scanStatus: .planned,
      mismatchSummary: "Captured value missing until custody label is assigned.",
      assignedOperatorTeam: "Receiving Desk",
      scanLocationStorageLocationID: storageLocations[2].id,
      custodyRecordID: custodyRecords[2].id,
      inventoryReceiptID: inventoryReceipts[2].id,
      orderID: nil,
      shipmentGroupID: nil,
      packageContentID: packageContents[2].id,
      evidenceAttachmentIDs: [],
      createdDate: "Today 11:15 AM",
      completedDate: "Not completed",
      notes: "Blocked local scan session because the label reference is missing a value.",
      riskLevel: .critical,
      reviewState: .needsReview
    )
  ]

  static var shipmentManifestRecords: [ShipmentManifestRecord] = [
    ShipmentManifestRecord(
      title: "Receiving desk courier handoff",
      manifestType: .courierHandoff,
      linkedEntityType: .scanSession,
      linkedEntityID: scanSessionRecords[0].id.uuidString,
      carrierCourier: "DHL Express",
      destinationSummary: "Melbourne office receiving desk",
      includedOrderIDs: [orders[0].id],
      shipmentGroupIDs: [shipmentGroups[0].id],
      inventoryReceiptIDs: [inventoryReceipts[0].id],
      packageContentIDs: [packageContents[0].id],
      custodyRecordIDs: [custodyRecords[0].id],
      labelReferenceIDs: [labelReferenceRecords[0].id],
      scanSessionIDs: [scanSessionRecords[0].id],
      evidenceAttachmentIDs: [evidenceAttachments[0].id],
      assignedOwnerTeam: "Receiving Desk",
      dispatchStatus: .blockedNeedsReview,
      plannedDispatchDate: "Today 4:00 PM",
      actualDispatchDate: "Not dispatched",
      handoffLocationStorageLocationID: storageLocations[0].id,
      manifestReferencePlaceholder: "MNF-DHL-001",
      notes: "Blocked until the damaged label mismatch is manually resolved.",
      riskLevel: .high,
      createdDate: "Today 11:10 AM",
      lastReviewedDate: "Needs review",
      reviewState: .needsReview
    ),
    ShipmentManifestRecord(
      title: "Packing bench internal delivery run",
      manifestType: .internalDeliveryRun,
      linkedEntityType: .storageLocation,
      linkedEntityID: storageLocations[1].id.uuidString,
      carrierCourier: "Internal courier",
      destinationSummary: "Packing bench to finance handoff desk",
      includedOrderIDs: orders.count > 1 ? [orders[1].id] : [],
      shipmentGroupIDs: [shipmentGroups[0].id],
      inventoryReceiptIDs: [inventoryReceipts[1].id],
      packageContentIDs: [packageContents[1].id],
      custodyRecordIDs: [custodyRecords[1].id],
      labelReferenceIDs: [labelReferenceRecords[1].id],
      scanSessionIDs: [scanSessionRecords[1].id],
      evidenceAttachmentIDs: [],
      assignedOwnerTeam: "ParcelOps Operations",
      dispatchStatus: .handedOff,
      plannedDispatchDate: "Yesterday 2:00 PM",
      actualDispatchDate: "Yesterday 2:15 PM",
      handoffLocationStorageLocationID: storageLocations[1].id,
      manifestReferencePlaceholder: "RUN-INT-014",
      notes: "Local handoff completed after shelf QR review.",
      riskLevel: .low,
      createdDate: "Yesterday 9:20 AM",
      lastReviewedDate: "Yesterday 2:30 PM",
      reviewState: .accepted
    ),
    ShipmentManifestRecord(
      title: "Outbound replacement transfer group",
      manifestType: .outboundTransferGroup,
      linkedEntityType: .custodyRecord,
      linkedEntityID: custodyRecords[2].id.uuidString,
      carrierCourier: "Australia Post",
      destinationSummary: "Replacement dispatch queue",
      includedOrderIDs: [],
      shipmentGroupIDs: [shipmentGroups[0].id],
      inventoryReceiptIDs: [inventoryReceipts[2].id],
      packageContentIDs: [packageContents[2].id],
      custodyRecordIDs: [custodyRecords[2].id],
      labelReferenceIDs: [labelReferenceRecords[2].id],
      scanSessionIDs: [scanSessionRecords[2].id],
      evidenceAttachmentIDs: [evidenceAttachments[1].id],
      assignedOwnerTeam: "Receiving Desk",
      dispatchStatus: .prepared,
      plannedDispatchDate: "Tomorrow 10:00 AM",
      actualDispatchDate: "Not dispatched",
      handoffLocationStorageLocationID: nil,
      manifestReferencePlaceholder: "MNF-AUSPOST-REP",
      notes: "Prepared locally but missing handoff location and final scan capture.",
      riskLevel: .critical,
      createdDate: "Today 12:20 PM",
      lastReviewedDate: "Needs review",
      reviewState: .needsReview
    )
  ]

  static var dispatchReadinessChecklists: [DispatchReadinessChecklist] = [
    DispatchReadinessChecklist(
      title: "DHL handoff readiness review",
      linkedEntityType: .shipmentManifest,
      linkedEntityID: shipmentManifestRecords[0].id.uuidString,
      shipmentManifestID: shipmentManifestRecords[0].id,
      orderIDs: shipmentManifestRecords[0].includedOrderIDs,
      shipmentGroupIDs: shipmentManifestRecords[0].shipmentGroupIDs,
      inventoryReceiptIDs: shipmentManifestRecords[0].inventoryReceiptIDs,
      packageContentIDs: shipmentManifestRecords[0].packageContentIDs,
      custodyRecordIDs: shipmentManifestRecords[0].custodyRecordIDs,
      labelReferenceIDs: shipmentManifestRecords[0].labelReferenceIDs,
      scanSessionIDs: shipmentManifestRecords[0].scanSessionIDs,
      evidenceAttachmentIDs: shipmentManifestRecords[0].evidenceAttachmentIDs,
      checklistType: .labelAndScan,
      checklistStatus: .blockedNeedsReview,
      requiredChecksSummary: "Confirm label mismatch resolution, custody owner, and handoff location before dispatch.",
      completedChecksSummary: "Manifest created and destination confirmed.",
      missingRequirementsSummary: "Damaged label mismatch still needs manual review.",
      assignedOwnerTeam: "Receiving Desk",
      plannedDispatchDate: shipmentManifestRecords[0].plannedDispatchDate,
      completedDate: "Not completed",
      riskLevel: .high,
    ...
    DispatchReadinessChecklist(
      title: "Internal delivery run ready check",
      linkedEntityType: .shipmentManifest,
      linkedEntityID: shipmentManifestRecords[1].id.uuidString,
      shipmentManifestID: shipmentManifestRecords[1].id,
      orderIDs: shipmentManifestRecords[1].includedOrderIDs,
      shipmentGroupIDs: shipmentManifestRecords[1].shipmentGroupIDs,
      inventoryReceiptIDs: shipmentManifestRecords[1].inventoryReceiptIDs,
      packageContentIDs: shipmentManifestRecords[1].packageContentIDs,
      custodyRecordIDs: shipmentManifestRecords[1].custodyRecordIDs,
      labelReferenceIDs: shipmentManifestRecords[1].labelReferenceIDs,
      scanSessionIDs: shipmentManifestRecords[1].scanSessionIDs,
      evidenceAttachmentIDs: shipmentManifestRecords[1].evidenceAttachmentIDs,
      checklistType: .custodyHandoff,
      checklistStatus: .completed,
      requiredChecksSummary: "Confirm internal handoff, shelf QR review, and recipient desk.",
      completedChecksSummary: "All local readiness checks completed.",
      missingRequirementsSummary: "No missing requirements.",
      assignedOwnerTeam: "ParcelOps Operations",
      plannedDispatchDate: shipmentManifestRecords[1].plannedDispatchDate,
      completedDate: "Yesterday 2:20 PM",
      riskLevel: .low,
      createdDate: "Yesterday 9:30 AM",
      lastReviewedDate: "Yesterday 2:35 PM",
      reviewState: .accepted
    ),
    DispatchReadinessChecklist(
      title: "Replacement transfer missing location",
      linkedEntityType: .shipmentManifest,
      linkedEntityID: shipmentManifestRecords[2].id.uuidString,
      shipmentManifestID: shipmentManifestRecords[2].id,
      orderIDs: shipmentManifestRecords[2].includedOrderIDs,
      shipmentGroupIDs: shipmentManifestRecords[2].shipmentGroupIDs,
      inventoryReceiptIDs: shipmentManifestRecords[2].inventoryReceiptIDs,
      packageContentIDs: shipmentManifestRecords[2].packageContentIDs,
      custodyRecordIDs: shipmentManifestRecords[2].custodyRecordIDs,
      labelReferenceIDs: shipmentManifestRecords[2].labelReferenceIDs,
      scanSessionIDs: shipmentManifestRecords[2].scanSessionIDs,
      evidenceAttachmentIDs: shipmentManifestRecords[2].evidenceAttachmentIDs,
      checklistType: .outboundTransfer,
      checklistStatus: .ready,
      requiredChecksSummary: "Confirm scan capture, included order, and handoff location before outbound transfer.",
      completedChecksSummary: "Manifest prepared and replacement package linked.",
      missingRequirementsSummary: "Handoff location and final captured scan value are missing.",
      assignedOwnerTeam: "Receiving Desk",
      plannedDispatchDate: shipmentManifestRecords[2].plannedDispatchDate,
      completedDate: "Not completed",
      riskLevel: .critical,
