...
protocol WishlistRepository {
  ...
  func loadWishlistSellerQuotes() -> [WishlistSellerQuote]
  func saveWishlistSellerQuotes(_ quotes: [WishlistSellerQuote])
  func loadWishlistPriceWatchRules() -> [WishlistPriceWatchRule]
  func saveWishlistPriceWatchRules(_ rules: [WishlistPriceWatchRule])
  func loadWishlistSellerTrustRecords() -> [WishlistSellerTrustRecord]
  func saveWishlistSellerTrustRecords(_ checks: [WishlistSellerTrustRecord])
  func loadWishlistPurchaseAccountRecords() -> [WishlistPurchaseAccountRecord]
  func saveWishlistPurchaseAccountRecords(_ records: [WishlistPurchaseAccountRecord])
  func loadWishlistPurchaseApprovalRecords() -> [WishlistPurchaseApprovalRecord]
...
final class JSONParcelOpsRepository: OrderRepository, MailEventRepository, IntakeEmailRepository, MailboxIngestRepository, IntegrationRepository, WishlistRepository, SettingsRepository, AuditRepository, EvidenceRepository, TrackingRepository, AutomationRuleRepository, SavedFilterRepository, ReviewTaskRepository, HandoffNoteRepository, SLAPolicyRepository, ExceptionPlaybookRepository, CommunicationRepository, ContactDirectoryRepository, CustomerRecipientProfileRepository, DestinationAddressRepository, DeliveryInstructionRepository, PackageContentRepository, CostRecordRepository, ReturnClaimRepository, ProcurementRequestRepository, ReceivingInspectionRepository, InventoryReceiptRepository, StorageLocationRepository, CustodyRepository, LabelReferenceRepository, ScanSessionRepository, ShipmentManifestRepository, DispatchReadinessRepository, AccountCredentialRepository, VendorProfileRepository, ShipmentGroupRepository, ImportQueueRepository, AcceptanceRepository {
  ...
  func loadSpaceMailIMAPConnections() -> [SpaceMailIMAPConnection] {
    load([SpaceMailIMAPConnection].self, from: .spaceMailIMAPConnections, defaultValue: SampleData.spaceMailIMAPConnections)
  }
  ...
  func loadWishlistResearchRequests() -> [WishlistResearchRequest] {
    load([WishlistResearchRequest].self, from: .wishlistResearchRequests, defaultValue: SampleData.wishlistResearchRequests)
  }
  ...
  func loadWishlistSellerQuotes() -> [WishlistSellerQuote] {
    load([WishlistSellerQuote].self, from: .wishlistSellerQuotes, defaultValue: SampleData.wishlistSellerQuotes)
  }

  func saveWishlistSellerQuotes(_ quotes: [WishlistSellerQuote]) {
    save(quotes, to: .wishlistSellerQuotes)
  }

  func loadWishlistPriceWatchRules() -> [WishlistPriceWatchRule] {
    load([WishlistPriceWatchRule].self, from: .wishlistPriceWatchRules, defaultValue: SampleData.wishlistPriceWatchRules)
  }

  func saveWishlistPriceWatchRules(_ rules: [WishlistPriceWatchRule]) {
    save(rules, to: .wishlistPriceWatchRules)
  }

  func loadWishlistSellerTrustRecords() -> [WishlistSellerTrustRecord] {
    load([WishlistSellerTrustRecord].self, from: .wishlistSellerTrustRecords, defaultValue: SampleData.wishlistSellerTrustRecords)
  }

  func saveWishlistSellerTrustRecords(_ checks: [WishlistSellerTrustRecord]) {
    save(checks, to: .wishlistSellerTrustRecords)
  }

  func loadWishlistPurchaseAccountRecords() -> [WishlistPurchaseAccountRecord] {
    load([WishlistPurchaseAccountRecord].self, from: .wishlistPurchaseAccountRecords, defaultValue: SampleData.wishlistPurchaseAccountRecords)
  }

  func saveWishlistPurchaseAccountRecords(_ records: [WishlistPurchaseAccountRecord]) {
    save(records, to: .wishlistPurchaseAccountRecords)
  }

  func loadWishlistPurchaseApprovalRecords() -> [WishlistPurchaseApprovalRecord] {
    load([WishlistPurchaseApprovalRecord].self, from: .wishlistPurchaseApprovalRecords, defaultValue: SampleData.wishlistPurchaseApprovalRecords)
  }

  func saveWishlistPurchaseApprovalRecords(_ records: [WishlistPurchaseApprovalRecord]) {
    save(records, to: .wishlistPurchaseApprovalRecords)
  }

  func loadWishlistPurchaseLinkRecords() -> [WishlistPurchaseLinkRecord] {
    load([WishlistPurchaseLinkRecord].self, from: .wishlistPurchaseLinkRecords, defaultValue: SampleData.wishlistPurchaseLinkRecords)
  }
  ...
  private enum StoreFile: String, CaseIterable {
    ...
    case wishlistSellerQuotes = "wishlist-seller-quotes.json"
    case wishlistPriceWatchRules = "wishlist-price-watch-rules.json"
    case wishlistSellerTrustRecords = "wishlist-seller-trust-checks.json"
...
final class InMemoryParcelOpsRepository: OrderRepository, MailEventRepository, IntakeEmailRepository, MailboxIngestRepository, IntegrationRepository, WishlistRepository, SettingsRepository, AuditRepository, EvidenceRepository, TrackingRepository, AutomationRuleRepository, SavedFilterRepository, ReviewTaskRepository, HandoffNoteRepository, SLAPolicyRepository, ExceptionPlaybookRepository, CommunicationRepository, ContactDirectoryRepository, CustomerRecipientProfileRepository, DestinationAddressRepository, DeliveryInstructionRepository, PackageContentRepository, CostRecordRepository, ReturnClaimRepository, ProcurementRequestRepository, ReceivingInspectionRepository, InventoryReceiptRepository, StorageLocationRepository, CustodyRepository, LabelReferenceRepository, ScanSessionRepository, ShipmentManifestRepository, DispatchReadinessRepository, AccountCredentialRepository, VendorProfileRepository, ShipmentGroupRepository, ImportQueueRepository, AcceptanceRepository {
  ...
  private var wishlistSellerQuotes = SampleData.wishlistSellerQuotes
  private var wishlistPriceWatchRules = SampleData.wishlistPriceWatchRules
  private var wishlistSellerTrustRecords = SampleData.wishlistSellerTrustRecords
  ...
  func loadWishlistSellerQuotes() -> [WishlistSellerQuote] { wishlistSellerQuotes }
  func saveWishlistSellerQuotes(_ quotes: [WishlistSellerQuote]) { wishlistSellerQuotes = quotes }
  func loadWishlistPriceWatchRules() -> [WishlistPriceWatchRule] { wishlistPriceWatchRules }
  func saveWishlistPriceWatchRules(_ rules: [WishlistPriceWatchRule]) { wishlistPriceWatchRules = rules }
  func loadWishlistSellerTrustRecords() -> [WishlistSellerTrustRecord] { wishlistSellerTrustRecords }
  func saveWishlistSellerTrustRecords(_ checks: [WishlistSellerTrustRecord]) { wishlistSellerTrustRecords = checks }
  func loadWishlistPurchaseAccountRecords() -> [WishlistPurchaseAccountRecord] { wishlistPurchaseAccountRecords }
  func saveWishlistPurchaseAccountRecords(_ records: [WishlistPurchaseAccountRecord]) { wishlistPurchaseAccountRecords = records }
  func loadWishlistPurchaseApprovalRecords() -> [WishlistPurchaseApprovalRecord] { wishlistPurchaseApprovalRecords }
